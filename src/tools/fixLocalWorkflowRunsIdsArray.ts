...
async function updateWorkflowFile(
  ...
  originalData: Zod.infer<typeof workflowStorage.schema>,
  update: Partial<Zod.infer<typeof workflowStorage.schema>>
) {
  const updatedData = {
    ...originalData,
    ...update,
...
function parseArgs() {
  ...
  const dryRun = args[3] === "--disable-dry-run" ? false : true;
